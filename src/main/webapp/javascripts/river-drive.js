...
var initRiverDrive = function(){
	...
	// Load rivers
	$.getJSON("api/2/repositories/rivers/drive",function(json) {
		// Handle errors
		if (!json.ok) {
			showRestError(json);
			return;
...
var openDriveRiver = function(id) {
	...
	$.getJSON("api/2/repositories/rivers/drive/" + riverId, function(json) {
		// Handle errors
		if (!json.ok) {
			showRestError(json);
			return;
...
var doCreateDriveRiver = function(e) {
	...
	data.start = false;
	$.postJSON("api/2/repositories/rivers/drive/", data, function(json) {
		// Handle errors
		if (!json.ok) {
			showRestError(json);
			return;
...
var doDeleteDriveRiver = function(e) {
	var data = getDriveRiver();
	$.ajax({
		url: "api/2/repositories/rivers/drive/" + data.id,
		type: "DELETE",
...
var doStartDriveRiver = function(e) {
	...
	var data = getDriveRiver();
	data.start = true;
	$.getJSON("api/2/repositories/rivers/drive/" + data.id + "/start", null, function (json){
		// Handle errors
		if (!json.ok) {
			showRestError(json);
			return;
...
var doStopDriveRiver = function(e) {
	...
	var data = getDriveRiver();
	data.start = false;
	$.getJSON("api/2/repositories/rivers/drive/" + data.id + "/stop", null, function (json){
		// Handle errors
		if (!json.ok) {
			showRestError(json);
			return;
