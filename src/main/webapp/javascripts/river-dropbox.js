...
var initRiverDropBox = function(){
	...
	// Load rivers
	$.getJSON("api/2/repositories/rivers/dropbox",function(json) {
		// Handle errors
		if (!json.ok) {
			showRestError(json);
			return;
...
var openDropBoxRiver = function(id) {
	...
	$.getJSON("api/2/repositories/rivers/dropbox/" + riverId, function(json) {
		// Handle errors
		if (!json.ok) {
			showRestError(json);
			return;
...
var doCreateDropBoxRiver = function(e) {
	...
	data.start = false;
	$.postJSON("api/2/repositories/rivers/dropbox/", data, function(json) {
		// Handle errors
		if (!json.ok) {
			showRestError(json);
			return;
...
var doDeleteDropBoxRiver = function(e) {
	var data = getDropBoxRiver();
	$.ajax({
		url: "api/2/repositories/rivers/dropbox/" + data.id,
		type: "DELETE",
...
var doStartDropBoxRiver = function(e) {
	...
	var data = getDropBoxRiver();
	data.start = true;
	$.getJSON("api/2/repositories/rivers/dropbox/" + data.id + "/start", null, function (json){
		// Handle errors
		if (!json.ok) {
			showRestError(json);
			return;
...
var doStopDropBoxRiver = function(e) {
	...
	var data = getDropBoxRiver();
	data.start = false;
	$.getJSON("api/2/repositories/rivers/dropbox/" + data.id + "/stop", null, function (json){
		// Handle errors
		if (!json.ok) {
			showRestError(json);
			return;
