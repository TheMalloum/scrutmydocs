...
var initRiverJira = function(){
	...
	// Load rivers
	$.getJSON("api/2/repositories/rivers/jira",function(json) {
		// Handle errors
		if (!json.ok) {
			showRestError(json);
			return;
...
var openJiraRiver = function(id) {
	...
	$.getJSON("api/2/repositories/rivers/jira/" + riverId, function(json) {
		// Handle errors
		if (!json.ok) {
			showRestError(json);
			return;
...
var doCreateJiraRiver = function(e) {
	...
	data.start = false;
	$.postJSON("api/2/repositories/rivers/jira/", data, function(json) {
		// Handle errors
		if (!json.ok) {
			showRestError(json);
			return;
...
var doDeleteJiraRiver = function(e) {
	var data = getJiraRiver();
	$.ajax({
		url: "api/2/repositories/rivers/jira/" + data.id,
		type: "DELETE",
...
var doStartJiraRiver = function(e) {
	...
	var data = getJiraRiver();
	data.start = true;
	$.getJSON("api/2/repositories/rivers/jira/" + data.id + "/start", null, function (json){
		// Handle errors
		if (!json.ok) {
			showRestError(json);
			return;
...
var doStopJiraRiver = function(e) {
	...
	var data = getJiraRiver();
	data.start = false;
	$.getJSON("api/2/repositories/rivers/jira/" + data.id + "/stop", null, function (json){
		// Handle errors
		if (!json.ok) {
			showRestError(json);
			return;
